...
package be.kjube.plugins.tablecompare;
...
public class TableCompare extends BaseStep implements StepInterface {
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
		...
		if (first) {
			...
			if (Const.isEmpty(meta.getReferenceSchemaField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.ReferenceSchemaNotSpecified"));
			}
			data.refSchemaIndex = getInputRowMeta().indexOfValue(meta.getReferenceSchemaField());
			if (data.refSchemaIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getReferenceSchemaField()));
			}

			// Reference table
			//
			if (Const.isEmpty(meta.getReferenceTableField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.ReferenceTableNotSpecified"));
			}
			data.refTableIndex = getInputRowMeta().indexOfValue(meta.getReferenceTableField());
			if (data.refTableIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getReferenceTableField()));
			}

			// Compare schema
			//
			if (Const.isEmpty(meta.getCompareSchemaField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CompareSchemaNotSpecified"));
			}
			data.cmpSchemaIndex = getInputRowMeta().indexOfValue(meta.getCompareSchemaField());
			if (data.cmpSchemaIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getCompareSchemaField()));
			}

			// Compare table
			//
			if (Const.isEmpty(meta.getCompareTableField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CompareTableNotSpecified"));
			}
			data.cmpTableIndex = getInputRowMeta().indexOfValue(meta.getCompareTableField());
			if (data.cmpTableIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getCompareTableField()));
			}

			// Key fields
			//
			if (Const.isEmpty(meta.getKeyFieldsField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.KeyFieldsNotSpecified"));
			}
			data.keyFieldsIndex = getInputRowMeta().indexOfValue(meta.getKeyFieldsField());
			if (data.keyFieldsIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getKeyFieldsField()));
			}

			// Exclude fields
			//
			if (Const.isEmpty(meta.getExcludeFieldsField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.ExcludeFieldsNotSpecified"));
			}
			data.excludeFieldsIndex = getInputRowMeta().indexOfValue(meta.getExcludeFieldsField());
			if (data.excludeFieldsIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getExcludeFieldsField()));
			}
			
			// error handling: Key description
			//
			if (Const.isEmpty(meta.getKeyDescriptionField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.KeyDescriptionFieldNotSpecified"));
			}
			data.keyDescIndex = getInputRowMeta().indexOfValue(meta.getKeyDescriptionField());
			if (data.keyDescIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getKeyDescriptionField()));
			}

			// error handling: reference value
			//
			if (Const.isEmpty(meta.getValueReferenceField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.ValueReferenceFieldNotSpecified"));
			}
			data.valueReferenceIndex = getInputRowMeta().indexOfValue(meta.getValueReferenceField());
			if (data.valueReferenceIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getValueReferenceField()));
			}

			// error handling: compare value
			//
			if (Const.isEmpty(meta.getValueCompareField())) {
				throw new KettleException(Messages.getString("TableCompare.Exception.ValueCompareFieldNotSpecified"));
			}
			data.valueCompareIndex = getInputRowMeta().indexOfValue(meta.getValueCompareField());
			if (data.valueCompareIndex<0) {
				throw new KettleException(Messages.getString("TableCompare.Exception.CanNotFindField", meta.getValueCompareField()));
			}
	...
	private Object[] compareTables(RowMetaInterface rowMeta, Object[] r) throws KettleException {
		try {
			...
			throw new KettleException(Messages.getString("TableCompare.Exception.UnexpectedErrorComparingTables"), e);
		}
	}
	...
	private Object[] compareTables(RowMetaInterface rowMeta, Object[] r, String referenceSchema, String referenceTable, String compareSchema, String compareTable, String keyFields, String excludeFields) throws KettleException {
		...
		if (Const.isEmpty(referenceTable)) {
			...
			putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Exception.NoReferenceTableDefined"), null, "TAC008");
			nrErrors++;
			;
		}
		...
		if (Const.isEmpty(compareTable)) {
			Object[] errorRowData = constructErrorRow(rowMeta, r, null, null, null);
			putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Exception.NoCompareTableDefined"), null, "TAC008");
			nrErrors++;
			;
		}
		...
		if (Const.isEmpty(keyFields)) {
			Object[] errorRowData = constructErrorRow(rowMeta, r, null, null, null);
			putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Exception.NoKeyFieldsDefined", refSchemaTable, cmpSchemaTable), null, "TAC007");
			nrErrors++;
			;
		}
		...
		try {
			...
			if (refFields.size() != cmpFields.size()) {
				Object[] errorRowData = constructErrorRow(rowMeta, r, null, null, null);
				putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Error.NumberOfFieldsIsDifferent", refSchemaTable, Integer.toString(refFields.size()), cmpSchemaTable, Integer.toString(cmpFields.size())), null, "TAC001");
				nrErrors++;
			} else {
				...
				for (String key : keys) {
					if (refFields.indexOfValue(key)<0) {
						if (getStepMeta().isDoingErrorHandling()) {
							Object[] errorRowData = constructErrorRow(rowMeta, r, null, null, null);
							putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Error.KeyFieldWasNotFoundInReferenceTable", key, refSchemaTable), null, "TAC002");
						}
						nrErrors++;
					}
				}
				for (String key : keys) {
					if (cmpFields.indexOfValue(key)<0) {
						if (getStepMeta().isDoingErrorHandling()) {
							Object[] errorRowData = constructErrorRow(rowMeta, r, null, null, null);
							putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Error.KeyFieldWasNotFoundInCompareTable", key, refSchemaTable), null, "TAC003");
						}
						nrErrors++;
					}
				}
				...
				do {
					...
					if (one==null && two!=null) {
						...
						if (getStepMeta().isDoingErrorHandling()) {
							...
							putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Error.RecordNotInReferenceFoundInCompareTable", cmpSchemaTable, keyRowMeta.getString(two)), null, "TAC004");
						}
						nrErrors++;
						nrRightErrors++;
						...
						if (getStepMeta().isDoingErrorHandling()) {
							...
							Object[] errorRowData = constructErrorRow(rowMeta, r, keyDesc, null, null);
							putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Error.RecordInReferenceNotFoundInCompareTable", refSchemaTable, keyRowMeta.getString(one)), null, "TAC005");
						}
						nrErrors++;
						nrLeftErrors++;
				                	...
				                	if (getStepMeta().isDoingErrorHandling()) {
						            	...
						            	for (int idx : valueNrs) {
						            		...
						            		if (cmp!=0) {
						            			...
						            			putError(data.errorRowMeta, errorRowData, 1, Messages.getString("TableCompare.Error.CompareRowIsDifferentFromReference"), valueMeta.getName(), "TAC006");
						            		}
						            	}
									}
									nrErrors++;
			...
			throw new KettleException(Messages.getString("TableCompare.Exception.UnexpectedErrorComparingTables", refSchemaTable, cmpSchemaTable), e);
		}
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		if (super.init(smi, sdi)) {
			...
			try {
				data.referenceDb = new Database(meta.getReferenceConnection());
				data.referenceDb.connect();
				
			} catch(Exception e) {
				logError(Messages.getString("TableCompare.Exception.UnexpectedErrorConnectingToReferenceDatabase", meta.getReferenceConnection().getName()), e);
				return false;
			}

			try {
				data.compareDb = new Database(meta.getCompareConnection());
				data.compareDb.connect();
				
			} catch(Exception e) {
				logError(Messages.getString("TableCompare.Exception.UnexpectedErrorConnectingToCompareDatabase", meta.getCompareConnection().getName()), e);
				return false;
			}
			return true;
		} 
