	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
				...
				if (!Const.isEmpty(meta.getFieldName())) {
					...
					for (int i = 0; i < meta.getFieldName().length; i++) {
						...
						if (data.fieldnrs[i] < 0) {
							logError(Messages.getString("Trim.Log.CanNotFindField", meta.getFieldName()[i]));
							throw new KettleException(Messages.getString("Trim.Log.CanNotFindField", meta.getFieldName()[i]));
						}
					}
				}
			}
		...
		try {
			...
			if (getStepMeta().isDoingErrorHandling()) {
				...
				logError(Messages.getString("Trim.Log.ErrorInStep", e.getMessage())); //$NON-NLS-1$
				e.printStackTrace();
				setErrors(1);
				stopAll();
