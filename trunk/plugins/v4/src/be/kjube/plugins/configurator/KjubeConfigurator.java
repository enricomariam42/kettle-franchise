...
@org.pentaho.di.core.annotations.JobEntry(
		...
		description="kJube.Plugins.KjubeConfigurator.Description"
	)
public class KjubeConfigurator extends JobEntryBase implements Cloneable, JobEntryInterface
{
	...
	public void saveRep(Repository rep, ObjectId id_job) throws KettleException {
		try
		{
			rep.saveJobEntryAttribute(id_job, getObjectId(), "customer_parameter",      customerParameterName);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "application_parameter",   applicationParameterName);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "lifecycle_parameter",     lifecycleParameterName);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "config_file_path",        configFilePath);
		}
	...
	public Result execute(Result previousResult, int nr)
	{
		...
		try {
			...
			if (Const.indexOfString(actualCustomerParameter, parameters)<0) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.ParameterNotDefined", actualCustomerParameter));
			}
			String customer =  parentJob.getParameterValue(actualCustomerParameter);
				
			if (Const.isEmpty(customer)) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.NoValueSetForParameter", actualCustomerParameter));
			}
			...
			if (Const.indexOfString(actualApplicationParameter, parameters)<0) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.ParameterNotDefined", actualApplicationParameter));
			}
			String application = parentJob.getParameterValue(actualApplicationParameter);
			if (Const.isEmpty(application)) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.NoValueSetForParameter", actualApplicationParameter));
			}
			...
			if (Const.indexOfString(actualLifecycleParameter, parameters)<0) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.ParameterNotDefined", actualLifecycleParameter));
			}
			String lifecycle = parentJob.getParameterValue(actualLifecycleParameter);
			if (Const.isEmpty(lifecycle)) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.NoValueSetForParameter", actualLifecycleParameter));
			}
			...
			if (!fileObject.exists()) {
				throw new KettleException(BaseMessages.getString(PKG, "KjubeConfigurator.Exception.ConfigurationFileNotFound", path, customer, application, lifecycle));
			}
			...
			while (keys.hasMoreElements()) {
				...
				if (!Const.isEmpty(key)) {
					...
					log.logDetailed(BaseMessages.getString(PKG, "KjubeConfigurator.Log.SetVariable", key, value));
				}
			}
			...
			log.logError(toString(), BaseMessages.getString(PKG, "KjubeConfigurator.Exception.UnexpectedError"), e);
