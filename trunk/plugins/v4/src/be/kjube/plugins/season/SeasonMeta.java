...
@Step(
		id = "SeasonId", 
		image = "be/kjube/plugins/images/season.png", 
		description = "kJube.Plugins.Season.Description", 
		name = "kJube.Plugins.Season.Name", 
		categoryDescription = "kJube.Category.Name", 
		i18nPackageName = "be.kjube.plugins"
	)
public class SeasonMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = Season.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$

	public enum ColumnType {

		CENTURIES(BaseMessages.getString(PKG, "Season.ColumnType.Centuries")), // $NON-NLS-1$
		YEARS(BaseMessages.getString(PKG, "Season.ColumnType.Years")), // $NON-NLS-1$
		SEASON_NUMBER(BaseMessages.getString(PKG, "Season.ColumnType.SeasonNumber")), // $NON-NLS-1$
		;
	...
	private List<SeasonField>	fields;
	...
	public Object clone() {
		SeasonMeta retval = (SeasonMeta) super.clone();
		retval.fields = new ArrayList<SeasonField>();
		retval.fields.addAll(fields);
		return retval;
	}

	public void allocate() {
		fields = new ArrayList<SeasonField>();
	}
	...
	public void getFields(RowMetaInterface inputRowMeta, String origin, RowMetaInterface[] info, StepMeta nextStep, VariableSpace space) throws KettleStepException {
		...
		for (SeasonField field : fields) {
			...
			//
			ValueMetaInterface seasonIdField = new ValueMeta(field.getResultField(), ValueMetaInterface.TYPE_INTEGER);
			seasonIdField.setLength(5, 0);
			inputRowMeta.addValueMeta(seasonIdField);
		}
	}
	...
	public String getXML() {
		...
		for (int i = 0; i < fields.size(); i++) {
			SeasonField field = fields.get(i);
			...
			retval.append("        " + XMLHandler.addTagValue("season_number", field.getSeasonNumberField()));
			retval.append("        </field>" + Const.CR);
		}
		retval.append("      </fields>" + Const.CR);
	...
	public void loadXML(Node stepnode, List<DatabaseMeta> databases, Map<String, Counter> counters) throws KettleXMLException {
		try {
			...
			for (int i = 0; i < nrfields; i++) {
				...
				String seasonNumberField = XMLHandler.getTagValue(fnode, "season_number");
				fields.add( new SeasonField(resultField, centuriesField, yearsField, seasonNumberField) );
			}
		} catch (Exception e) {
    ...
    public void readRep(Repository rep, ObjectId id_step, List<DatabaseMeta> databases, Map<String, Counter> counters) throws KettleException {
		try {
			...
			for (int i = 0; i < nrfields; i++) {
				...
				String seasonNumberField = rep.getStepAttributeString(id_step, i, "field_season_number");
				fields.add( new SeasonField(resultField, centuriesField, yearsField, seasonNumberField) );
			}
		} catch (Exception e) {
	...
	public void saveRep(Repository rep, ObjectId id_transformation, ObjectId id_step) throws KettleException {
		try {
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i, "field_season_number", field.getSeasonNumberField());
			}
		} catch (Exception e) {
	...
	public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta tr, Trans trans) {
		return new Season(stepMeta, stepDataInterface, cnr, tr, trans);
	}
	 ...
	 * @return the fields
	 */
	public List<SeasonField> getFields() {
		return fields;
	}
	 ...
	 *            the fields to set
	 */
	public void setFields(List<SeasonField> fields) {
		this.fields = fields;
	}
