...
public class Rejects extends BaseStep implements StepInterface {
		...
		if (first) {
			...
			for (String keyField : meta.getKeyFields()) {
				...
				if (!Const.isEmpty(key)) {
					...
					if (index<0) {
						throw new KettleException(BaseMessages.getString(PKG, "Rejects.Log.CanNotFindField", keyField));
					}
			...
			if (prevStepNames==null || prevStepNames.length==0) {
				throw new KettleException(BaseMessages.getString(PKG, "Rejects.Log.CanNotFindPreviousSteps"));
			}
			data.previousSteps = "";
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		if (super.init(smi, sdi)) {
			...
			data.db = new Database(this, dbMeta);
			try {
