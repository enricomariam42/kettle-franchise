...
public class DataGrid extends BaseStep implements StepInterface
{
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException
	{
        ...
        if (log.isRowLevel())
        {
            log.logRowlevel(toString(), Messages.getString("DataGrid.Log.Wrote.Row", Long.toString(getLinesWritten()), data.outputRowMeta.getString(outputRowData)) );
        }
        
        if (checkFeedback(getLinesWritten())) 
        {
        	if(log.isBasic()) logBasic( Messages.getString("DataGrid.Log.LineNr", Long.toString(getLinesWritten()) ));
        }
