...
@Step(
		...
		tooltip = "kJube.Plugins.Rejects.Description", 
		description = "kJube.Plugins.Rejects.Name", 
...
public class RejectsMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public void readRep(Repository rep, long id_step, List<DatabaseMeta> databases, Map<String, Counter> counters) throws KettleException {
		try {
			databaseMeta = DatabaseMeta.findDatabase(databases, rep.getStepAttributeString(id_step, "database"));
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepinfo, RowMetaInterface prev, String input[], String output[], RowMetaInterface info) {
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, Messages.getString("RejectsMeta.CheckResult.StepRecevingData2"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, Messages.getString("RejectsMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
